// Render reel.html (1080x1920, 30fps) to MP4 (H.264 + AAC) with headless Chromium.
//   node render.mjs                     full render -> reel.mp4
...
const url = pathToFileURL(join(here, 'reel.html')).href + '?render=1';
let FPS = 30, FRAMES = 900; // overwritten from window.META
...
async function page() {
  ...
  ({ FPS, FRAMES } = await p.evaluate(() => window.META));
  return p;
}
...
const audio = join(here, 'reel-audio.wav');
